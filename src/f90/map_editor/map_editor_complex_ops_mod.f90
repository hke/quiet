...
contains
  ...
  subroutine output_harmonic_mode
    implicit none

    character(len=256) :: tmp, outmap
    integer(i4b)       :: l, m, pol, nside, npix
    real(dp),     allocatable, dimension(:,:)   :: map
    complex(dpc), allocatable, dimension(:,:,:) :: alm

    call getarg(2,outmap)
    call getarg(3,tmp)
    read(tmp,*) l
    call getarg(4,tmp)
    read(tmp,*) m
    ...
    npix = 12*nside**2
    write(*,*) 'b'
    allocate(map(0:npix-1,3), alm(3,0:l,0:l))
    write(*,*) 'c'
    alm          = 0.d0
    write(*,*) 'd'
    if (m >= 0.d0) then
       alm(pol,l,m) = cmplx(1.d0,0.d0)
    else
       write(*,*) 'e'
       alm(pol,l,abs(m)) = cmplx(0.d0,1.d0)
       write(*,*) 'f'
    end if
    ...
    call write_map(map, 1, outmap)
    write(*,*) 'h'

  end subroutine output_harmonic_mode

  subroutine pixconv
    implicit none

    real(dp)           :: theta_max, vec(3), theta, vec0(3), weight, w
    integer(i4b)       :: lmax, i, j, l, nbin, nspline, unit, order, nside_out, nside_in, n
    character(len=512) :: inbeam, inmap, outfile, tmp
    integer(i4b), allocatable, dimension(:) :: pixlist
    real(dp), allocatable, dimension(:)   :: bl, pl
    real(dp), allocatable, dimension(:,:) :: beam, b_spline, map, outmap
    type(spline_type)  :: s

    if (iargc() < 6) then
       write(*,*) 'Usage: map_editor pixconv [inmap] [beam realspace] [nbin beam] [nside_out] [outmap] '
       stop
    end if

    call getarg(2,inmap)
    call getarg(3,inbeam)
    call getarg(4,tmp)
    read(tmp,*) nbin
    call getarg(5,tmp)
    read(tmp,*) nside_out
    ...
    unit = 21
    allocate(beam(nbin,2), bl(0:lmax), b_spline(nspline,2), pl(0:lmax))

    open(unit,file=trim(inbeam))
    do i = 1, nbin
       read(unit,*) beam(i,1), beam(i,2)
       beam(i,1) = beam(i,1) * pi / 180.d0 ! Radians
    end do
    close(unit)
    theta_max = maxval(beam(:,1))

    call spline(s, beam(:,1), beam(:,2))
    do i = 1, nspline
       b_spline(i,1) = theta_max / (nspline-1.d0) * real(i-1,dp)
       b_spline(i,2) = splint(s, b_spline(i,1))
    end do

    call read_map(map, order, inmap)
    nside_in = nint(sqrt(size(map,1)/12.d0))
    allocate(outmap(0:12*nside_out**2-1,1), pixlist(0:12*nside_in**2-1))
    outmap = 0.d0
    do i = 0, 12*nside_out**2-1
       if (mod(i,100) == 0) write(*,*) 'pixel ', i, ' of ', 12*nside_out**2
       call pix2vec_nest(nside_out, i, vec0)
       call query_disc(nside_in, vec0, theta_max, pixlist, n, nest=1)
       weight = 0.d0
       do j = 0, n-1
          call pix2vec_nest(nside_in, pixlist(j), vec)
          call angdist(vec0, vec, theta)
          w = splint(s, theta)
          outmap(i,1) = outmap(i,1) + map(pixlist(j),1) * w
          weight = weight + w
       end do
       outmap(i,1) = outmap(i,1) / weight
    end do

    call write_map(outmap, order, outfile)


  end subroutine pixconv


  subroutine pixconv_firas
    implicit none

    real(dp)           :: theta_max, vec(3), theta, vec0(3), weight, w, vec_rot(3), vec1(3)
    real(dp)           :: M_tot(3,3), M_ecl(3,3), M_rot(3,3), phi_ecl, theta_ecl, dalpha0, alpha0, dalpha1, alpha1
    real(dp)           :: phi0, phi1, phi_rot, theta_rot, M_sat(3,3), alpha
    integer(i4b)       :: lmax, i, j, k, l, nbin, nspline, unit, order, nside_out, nside_in, n, nstep, q
    character(len=512) :: inbeam, inmap, outfile, tmp
    integer(i4b), allocatable, dimension(:) :: pixlist
    real(dp), allocatable, dimension(:)   :: bl, pl
    real(dp), allocatable, dimension(:,:) :: beam, b_spline, map, outmap
    ...
    !nstep   = 1
    !alpha0  =   0.d0 * pi/180.d0
    !dalpha  = 0.26d0 * pi/180.d0
    !phi0    =  45.d0 * pi/180.d0
    ...
    call compute_euler_matrix_zyz(-phi_ecl, -theta_ecl, 0.d0, M_ecl)
    ...
    call spline(s, beam(:,1), beam(:,2))
    open(unit,file='beam_spline.dat')
    do i = 1, nspline
       b_spline(i,1) = theta_max / (nspline-1.d0) * real(i-1,dp)
       b_spline(i,2) = splint(s, b_spline(i,1))
       write(unit,*) b_spline(i,1)*180/pi, b_spline(i,2)
    end do
    close(unit)
...
!!$    map = 0.d0
!!$    do i = 0, size(map,1)-1
!!$       call pix2vec_nest(nside_in, i, vec0)
!!$       map(i,1) = vec0(3)
!!$    end do
    ...
    call apply_pixconv(s, map, outmap, alpha0, dalpha0, alpha1, dalpha1)

!!$    outmap = 0.d0
!!$    do i = 0, 12*nside_out**2-1
!!$       if (mod(i,100) == 0) write(*,*) i, 12*nside_out**2
!!$       weight = 0.d0
!!$       call pix2vec_nest(nside_out, i, vec0)
!!$       vec0 = matmul(transpose(M_ecl), vec0)
!!$       !vec0 = [0.d0, 1.d0, 0.d0]
!!$       !write(*,*) 'vec0 = ', real(vec0,sp)
!!$       vec_rot   = crossprod(vec0, [0.d0,0.d0,1.d0])
!!$       vec_rot   = vec_rot / sqrt(sum(vec_rot**2))
!!$       !write(*,*) 'rot = ', real(vec_rot,sp)
...
!!$       do k = 1, nstep
!!$          alpha = alpha0 + (k-1)*dalpha
!!$          if (modulo(phi_ecl-phi0,2.d0*pi) > pi) alpha = -alpha
!!$          call compute_euler_matrix_zyz(0.d0, 0.d0, alpha, M_sat)
!!$          vec1 = matmul(M_ecl, matmul(M_rot, matmul(M_sat, &
!!$               & matmul(transpose(M_rot), vec0))))
!!$          call query_disc(nside_in, vec1, theta_max, pixlist, n, nest=1)
!!$          do j = 0, n-1
!!$             call pix2vec_nest(nside_in, pixlist(j), vec)
...
!!$    end do
    ...
    call write_map(map, order, 'map.fits')


  end subroutine pixconv_firas

  subroutine apply_pixconv(s, inmap, outmap, alpha0, dalpha0, alpha1, dalpha1)
    implicit none
    type(spline_type),                   intent(in)  :: s
    real(dp),          dimension(0:,1:), intent(in)  :: inmap
    real(dp),          dimension(0:,1:), intent(out) :: outmap
    real(dp),                            intent(in)  :: alpha0, dalpha0, alpha1, dalpha1

    integer(i4b) :: nside_in, nside_out, i, j, k, l, n, nstep
    real(dp)     :: phi_ecl, theta_ecl, M_ecl(3,3), M_rot(3,3), M_sat(3,3), phi0, phi1
    real(dp)     :: weight, w, vec(3), vec0(3), vec_rot(3), vec1(3)
    real(dp)     :: theta
    real(dp)     :: phi_rot, theta_rot, alpha, theta_max
    integer(i4b), allocatable, dimension(:) :: pixlist
    real(dp),     allocatable, dimension(:,:), save :: vecs

    phi0    =  200.d0 * pi/180.d0
    phi1    =  350.d0 * pi/180.d0
    
    nside_in  = nint(sqrt(size(inmap,1)/12.d0))
    nside_out = nint(sqrt(size(outmap,1)/12.d0))
    theta_max = maxval(s%x)

    if (.not. allocated(vecs)) then
       allocate(vecs(0:12*nside_in**2-1,3))
       do i = 0, 12*nside_in**2-1
          call pix2vec_nest(nside_in, i, vecs(i,:))
       end do
    end if
    ...
    allocate(pixlist(0:12*nside_in**2-1))
    outmap = 0.d0
    do i = 0, 12*nside_out**2-1
       if (mod(i,1000) == 0) write(*,*) i, 12*nside_out**2
       weight = 1.d0
       call pix2vec_nest(nside_out, i, vec0)
       vec0 = matmul(transpose(M_ecl), vec0)
       vec_rot   = crossprod(vec0, [0.d0,0.d0,1.d0])
       ...
       do k = 1, nstep
          ...
          do j = 0, n-1
             ...
             call angdist(vec1, vec, theta)
             w = splint(s, theta)
             outmap(i,1) = outmap(i,1) + inmap(pixlist(j),1) * w
             weight = weight + w
          end do
       end do
       outmap(i,1) = outmap(i,1) / weight
    ...
    deallocate(pixlist)
  end subroutine apply_pixconv


  subroutine template_fit
    implicit none

    real(dp)           :: theta_max, vec(3), theta, vec0(3), weight, w, nullval
    logical(lgt)       :: anynull
    integer(i4b)       :: lmax, i, j, k, l, nbin, nspline, unit, order, nside, ordering, n, p, npix, nmaps
    character(len=512) :: infile, outfile, maskfile, tmp
    integer(i4b), allocatable, dimension(:) :: pixlist
    real(dp), allocatable, dimension(:)     :: bl, pl, b
    real(dp), allocatable, dimension(:,:)   :: beam, b_spline, map, outmap, A, mask
    real(dp), allocatable, dimension(:,:,:) :: T
    type(spline_type)  :: s

    if (iargc() < 5) then
       write(*,*) 'Usage: map_editor template_fit [infile] [maskfile] [outfile] [template 1] [template 2] [...] '
       stop
    end if
    ...
    i = getsize_fits(maskfile, nside=nside, ordering=ordering, nmaps=nmaps)
    npix = nside2npix(nside)
    ...
    n = iargc() - 4 !+ 1
    allocate(T(0:npix-1,nmaps,n))
    do i = 1, n!-1
       call getarg(4+i,tmp)
       j = getsize_fits(tmp, nside=nside, ordering=ordering, nmaps=nmaps)
       call read_bintab(tmp, T(:,:,i), npix, nmaps, nullval, anynull)
       if (ordering == 2) then
          do j = 1, nmaps
             call convert_nest2ring(nside, T(:,j,i))
          end do
       end if
    end do
    !T(:,1,n) = 1.d0
    ...
    do k = 1, nmaps
       do p = 0, npix-1
          if (mask(p,k) == 0 .or. map(p,k) < -1.63d30) cycle
          do i = 1, n
             do j = 1, n
                A(i,j) = A(i,j) + mask(p,k) * T(p,k,i) * T(p,k,j)
             end do
             b(i) = b(i) + mask(p,k) * T(p,k,i) * map(p,k)
          end do
       end do
    end do
...
!    end where
    ...
    call write_map(map, 1, outfile)

    map = 0.d0
    do i = 1, n
       map = map + b(i) * T(:,:,i)
    end do
    call write_map(map, 1, 'temp_corr.fits')
  ...
  subroutine fit_gain_offset
    ...
    if (iargc() /= 6) then
       write(*,*) 'Usage: map_editor fit_gain_offset [xmap] [ymap] [gain mask] [offset mask] [output residualfile]'
       stop
    end if
...
91  close(unit)
  ...
  subroutine make_merged_ptsrc_map
    implicit none

    type src
       real(dp)          :: lat, lon, ra(3), dec(3), flux, beta, S_nvss, vec(3)
       logical(lgt)      :: included
       character(len=24) :: label
    end type src

    integer(i4b)       :: i, j, k, n, nside, npix, nmaps, unit, col, nlist, nsrc, n_at20g, n_pccs2, n_nvss, n_threshold, n_gb6
    character(len=128) :: partext, convention, infile, outfile, beamfile, outprefix
    character(len=1024) :: line, at20gfile, pccs2file, nvssfile, gb6file
    real(dp)           :: vec(3), cvec(3), r, sigma, dOmega, Omega, fwhm, norm, at20g_threshold
    real(dp)           :: u(3), v(3), len_u, len_v, theta, flux, z, sgn, a, b, c, d, e, f, dec_lim
    real(dp)           :: at20g_dens, nvss_cut, nvss_threshold, pccs2_angdist, dist
    real(dp)           :: gb6_cut, gb6_threshold
    integer(i4b), allocatable, dimension(:)   :: listpix
    real(dp),     allocatable, dimension(:,:) :: map
    real(dp),     allocatable, dimension(:)   :: nvss_fluxes, gb6_fluxes
    type(src),    allocatable, dimension(:)   :: at20g, nvss, pccs2, gb6
    real(dp),     allocatable, dimension(:)   :: data, srcmap
    real(dp),     allocatable, dimension(:)   :: b_r, b_prof, b_prof2
    character(len=80), dimension(180)         :: header

    if (iargc() < 9) then
       write(*,*) 'Usage: map_editor make_ptsrc_map [AT20G catalog] [PCCS2 catalog] [NVSS catalog] [GB6 catalog]'
       write(*,*) '            [nside] [fwhm] [AT20G threshold] [outprefix]'
       stop
    end if
    
    call getarg(2,at20gfile)
    call getarg(3,pccs2file)
    call getarg(4,nvssfile)
    call getarg(5,gb6file)
    call getarg(6,partext)
    read(partext,*) nside
    call getarg(7,partext)
    read(partext,*) fwhm
    call getarg(8,partext)
    read(partext,*) at20g_threshold
    ...
    dec_lim = -15.d0   ! Declination cut between AT20G and NVSS
    n_at20g = 5890
    n_pccs2 = 1560
    n_nvss  = 1773484
    n_gb6   = 75162
    pccs2_angdist = cos(10.d0*pi/180.d0/60.d0) ! Maximum distance in arcmin

    allocate(map(0:npix-1,6), listpix(0:npix-1), srcmap(0:npix-1))
    allocate(data(7), pccs2(n_pccs2), nvss(n_nvss), at20g(n_at20g), gb6(n_gb6))

    ! Read AT20G
    open(58,file=trim(at20gfile), recl=1024)
    do i = 1, n_at20g
       read(58,*) at20g(i)%lon, at20g(i)%lat, at20g(i)%label, at20g(i)%ra, &
            & at20g(i)%dec, at20g(i)%flux, at20g(i)%S_nvss
       call ang2vec((90.d0-at20g(i)%lat)*DEG2RAD, at20g(i)%lon*DEG2RAD, at20g(i)%vec)
    end do
    close(58)

    ! Read PCCS2
    open(58,file=trim(pccs2file), recl=1024)
    do i = 1, n_pccs2
       read(58,*) pccs2(i)%lon, pccs2(i)%lat, pccs2(i)%flux, a, b, c, d, e, pccs2(i)%label
       call ang2vec((90.d0-pccs2(i)%lat)*DEG2RAD, pccs2(i)%lon*DEG2RAD, pccs2(i)%vec)
    end do
    close(58)

    ! Read NVSS
    open(58,file=trim(nvssfile), recl=1024)
    do i = 1, n_nvss
       read(58,*) nvss(i)%lon, nvss(i)%lat, nvss(i)%label, nvss(i)%ra, &
            & nvss(i)%dec, nvss(i)%flux 
       call ang2vec((90.d0-nvss(i)%lat)*DEG2RAD, nvss(i)%lon*DEG2RAD, nvss(i)%vec)
    end do
    close(58)

    ! Read GB6
    open(58,file=trim(gb6file), recl=1024)
    do i = 1, n_gb6
       read(58,*) gb6(i)%lon, gb6(i)%lat, gb6(i)%ra, &
            & gb6(i)%dec, gb6(i)%flux, gb6(i)%label
       call ang2vec((90.d0-gb6(i)%lat)*DEG2RAD, gb6(i)%lon*DEG2RAD, gb6(i)%vec)
    end do
    close(58)
    


    ! Step 1: Threshold AT20G at requested value, and compute the appropriate number of NVSS sources to include
    n_threshold = 0
    do i = 1, n_at20g
       if (at20g(i)%flux > at20g_threshold .and. at20g(i)%dec(1) <= dec_lim) then
          n_threshold = n_threshold+1
          at20g(i)%included = .true.
       else
          at20g(i)%included = .false.
       end if
    end do
    at20g_dens = n_threshold / (2.d0*pi*(1.d0-cos((90.d0+dec_lim)*pi/180.d0)))

    
    ! Step 2: Threshold GB6 at predicted value
    !gb6_cut   = at20g_dens * (2.d0*pi-(2.d0*pi*(1.d0-cos((90.d0-15)*pi/180.d0))))
    gb6_cut   = at20g_dens * 2.d0*pi*cos(15*pi/180.d0)
    allocate(gb6_fluxes(n_gb6))
    gb6_fluxes = gb6%flux
    call QuickSort_real(gb6_fluxes)
    gb6_threshold = gb6_fluxes(n_gb6-floor(gb6_cut)+1)
    do i = 1, n_gb6
       if (gb6(i)%flux > gb6_threshold) then
          gb6(i)%included = .true.
       else
          gb6(i)%included = .false.
       end if
    end do

    ! Step 3: Threshold NVSS at predicted value
    nvss_cut   = at20g_dens * (2.d0*pi*(1.d0-cos((90.d0-dec_lim)*pi/180.d0)))
    allocate(nvss_fluxes(n_nvss))
    nvss_fluxes = nvss%flux
    call QuickSort_real(nvss_fluxes)
    nvss_threshold = nvss_fluxes(n_nvss-floor(nvss_cut)+1)
    do i = 1, n_nvss
       if (nvss(i)%flux > nvss_threshold &
            & .and. (nvss(i)%dec(1) > dec_lim .or. abs(nvss(i)%lat) < 1.5d0) .and. &
            & (nvss(i)%dec(1) >= 75.d0 .or. nvss(i)%dec(1) < 0.d0)) then
          nvss(i)%included = .true.
       else
          nvss(i)%included = .false.
       end if
    end do

    ! Step 4: Include any PCCS2 source that is not already included
    pccs2%included = .true.
    do j = 1, n_at20g
       if (.not. at20g(j)%included) cycle
       do i = 1, n_pccs2
          if (sum(pccs2(i)%vec*at20g(j)%vec) > pccs2_angdist) pccs2(i)%included = .false.
       end do
    end do

    do j = 1, n_nvss
       if (.not. nvss(j)%included) cycle
       do i = 1, n_pccs2
          if (sum(pccs2(i)%vec*nvss(j)%vec) > pccs2_angdist) pccs2(i)%included = .false.
       end do
    end do

    do j = 1, n_gb6
       if (.not. gb6(j)%included) cycle
       do i = 1, n_pccs2
          if (sum(pccs2(i)%vec*gb6(j)%vec) > pccs2_angdist) pccs2(i)%included = .false.
       end do
    end do


    open(68,file=trim(outprefix)//'.dat', recl=1024)
    ...
    ! AT20G
    do i = 1, n_at20g
       if (.not. at20g(i)%included) cycle
       flux      = at20g(i)%flux
       nsrc = nsrc+1
       if (mod(nsrc,1000) == 0) write(*,*) 'Processing source no. ', nsrc
       call query_disc(nside, at20g(i)%vec, 2*fwhm, listpix, nlist)
       srcmap = 0.d0
       do j = 0, nlist-1
          call pix2vec_ring(nside, listpix(j), vec)
          r            = acos(sum(vec*at20g(i)%vec))
          srcmap(j)    = exp(-0.5d0 * (r/sigma)**2)
       end do
          
       ! Listed values are peak flux density
       norm           = max(flux,0.d0) / (sum(srcmap(0:nlist-1))*dOmega)
       srcmap(0:nlist-1) = norm * srcmap(0:nlist-1)
       map(listpix(0:nlist-1),1) = map(listpix(0:nlist-1),1) + srcmap(0:nlist-1)
       map(listpix(0:nlist-1),2) = map(listpix(0:nlist-1),2) + srcmap(0:nlist-1)
       map(listpix(0:nlist-1),6) = 1.d0
       
       write(68,fmt='(2f8.3,2f16.3,4f8.3,a,a)') at20g(i)%lon, at20g(i)%lat, at20g(i)%flux, &
            & 0.2d0*at20g(i)%flux, 0.d0, 0.d0, 0.d0, 0.d0, at20g(i)%label, '  AT20G'
    end do

    ! NVSS
    do i = 1, n_nvss
       if (.not. nvss(i)%included) cycle
       flux      = nvss(i)%flux
       nsrc = nsrc+1
       if (mod(nsrc,1000) == 0) write(*,*) 'Processing source no. ', nsrc
       call query_disc(nside, nvss(i)%vec, 2*fwhm, listpix, nlist)
       srcmap = 0.d0
       do j = 0, nlist-1
          call pix2vec_ring(nside, listpix(j), vec)
          r            = acos(sum(vec*nvss(i)%vec))
          srcmap(j)    = exp(-0.5d0 * (r/sigma)**2)
       end do
       ...
       srcmap(0:nlist-1) = norm * srcmap(0:nlist-1)
       map(listpix(0:nlist-1),1) = map(listpix(0:nlist-1),1) + srcmap(0:nlist-1)
       map(listpix(0:nlist-1),3) = map(listpix(0:nlist-1),3) + srcmap(0:nlist-1)
       map(listpix(0:nlist-1),6) = 2.d0

       write(68,fmt='(2f8.3,2f16.3,4f8.3,a,a)') nvss(i)%lon, nvss(i)%lat, nvss(i)%flux, &
            & 0.2d0*nvss(i)%flux, 0.d0, 0.d0, 0.d0, 0.d0, nvss(i)%label, '  NVSS'
    end do

    ! GB6
    do i = 1, n_gb6
       if (.not. gb6(i)%included) cycle
       flux      = gb6(i)%flux
       nsrc = nsrc+1
       if (mod(nsrc,1000) == 0) write(*,*) 'Processing source no. ', nsrc
       call query_disc(nside, gb6(i)%vec, 2*fwhm, listpix, nlist)
       srcmap = 0.d0
       do j = 0, nlist-1
          call pix2vec_ring(nside, listpix(j), vec)
          r            = acos(sum(vec*gb6(i)%vec))
          srcmap(j)    = exp(-0.5d0 * (r/sigma)**2)
       end do
       ...
       srcmap(0:nlist-1) = norm * srcmap(0:nlist-1)
       map(listpix(0:nlist-1),1) = map(listpix(0:nlist-1),1) + srcmap(0:nlist-1)
       map(listpix(0:nlist-1),4) = map(listpix(0:nlist-1),4) + srcmap(0:nlist-1)
       map(listpix(0:nlist-1),6) = 3.d0

       write(68,fmt='(2f8.3,2f16.3,4f8.3,a,a)') gb6(i)%lon, gb6(i)%lat, gb6(i)%flux, &
            & 0.2d0*gb6(i)%flux, 0.d0, 0.d0, 0.d0, 0.d0, gb6(i)%label, '  GB6'
    end do

    ! PCCS2
    do i = 1, n_pccs2
       if (.not. pccs2(i)%included) cycle
       flux      = pccs2(i)%flux
       nsrc = nsrc+1
       if (mod(nsrc,1000) == 0) write(*,*) 'Processing source no. ', nsrc
       call query_disc(nside, pccs2(i)%vec, 2*fwhm, listpix, nlist)
       srcmap = 0.d0
       do j = 0, nlist-1
          call pix2vec_ring(nside, listpix(j), vec)
          r            = acos(sum(vec*pccs2(i)%vec))
          srcmap(j)    = exp(-0.5d0 * (r/sigma)**2)
       end do
       ...
       srcmap(0:nlist-1) = norm * srcmap(0:nlist-1)
       map(listpix(0:nlist-1),1) = map(listpix(0:nlist-1),1) - srcmap(0:nlist-1)
       map(listpix(0:nlist-1),5) = map(listpix(0:nlist-1),5) + srcmap(0:nlist-1)
       map(listpix(0:nlist-1),6) = 10.d0

       write(68,fmt='(2f8.3,2f16.3,4f8.3,a,a)') pccs2(i)%lon, pccs2(i)%lat, pccs2(i)%flux, &
            & 0.2d0*pccs2(i)%flux, 0.d0, 0.d0, 0.d0, 0.d0, pccs2(i)%label, '  PCCS2'
    end do
    close(68)
    ...
    write(*,*) 'Number of AT20G sources = ', count(at20g%included)
    write(*,*) 'Number of NVSS sources  = ', count(nvss%included)
    write(*,*) 'Number of GB6 sources   = ', count(gb6%included)
    write(*,*) 'Number of PCCS2 sources = ', count(pccs2%included)
    write(*,*) 'Total number of sources = ', count(pccs2%included)+count(at20g%included)+count(nvss%included)+count(gb6%included)

  end subroutine make_merged_ptsrc_map
...
!       call input_map(filenames(i), map, npix, nmaps)
  ...
  subroutine smooth_map(infile, r_fill, lmin, lmax, nside, ordering, nmaps, map, header, &
       & input_EB, output_EB, beamfile_in, beamfile_out, fwhm_in, fwhm_out)
    implicit none
    ...
    logical(lgt),                               intent(in)           :: input_EB, output_EB
    real(dp),           pointer, dimension(:,:)                      :: map
    character(len=80),           dimension(180)                      :: header

    integer(i4b)       :: nside_in, npix, npix_in
    integer(i4b)       :: i, j, k, l, m, r, nlist
    real(dp)           :: nullval, tot, cl
    ...
    if (nmaps == 1) then
       ...
       if (input_EB) then
          do i = 1, nmaps
             call map2alm(nside_in, lmax, lmax, map_in(:,i), alms(i:i,:,:), zbounds, weights(:,1:1))
          end do
       else
    ...
    do i = 1, nmaps
       do l = 2, lmax
          cl = abs(alms(i,l,0))**2
          do m = 1, l
             cl = cl + 2.d0*abs(alms(i,l,m))**2
          end do
          if ((pixwin_out(l,i)*beam_out(l,i))**2 > 1e-12) then
             cl = cl / real(2*l+1,dp) * l*(l+1)/(2.d0*pi) / (pixwin_out(l,i)*beam_out(l,i))**2
          else
             cl = 0.d0
          end if
          write(58,*) l, cl
...
1010 if (ordering == 1) then
  ...
  subroutine print_stats(mapfile, maskfile)
    ...
    real(dp)     :: theta, phi, t1, t2, integral, radius, mu, sigma, my_min, my_max, scale, kappa, gamma
    logical(lgt) :: anynull
    type(planck_rng) :: rng_handle
    ...
    !do i = 2, 2
       ...
       kappa = sum(vals(1:k)**3)/(k-1)
       gamma = sum((vals(1:k)-mu)**4)/(k-1)
       if (iargc()==4) then
          ...
          write(*,*) '   Mean = ', mu
          write(*,*) '   RMS  = ', sigma
          write(*,*) '   Skew = ', (kappa - 3*mu*sigma**2 - mu**3) / sigma**3
          write(*,*) '   Kurt = ', gamma / sigma**4 - 3
...
!    write(*,*) 'multipoles = ', multipoles
  ...
  subroutine print_pol_angles(mapfile1, mapfile2, maskfile, mapfile_out)
    implicit none

    character(len=128), intent(in) :: mapfile1, mapfile2, maskfile, mapfile_out

    integer(i4b) :: npix, nside, ordering, nmaps, i, j, ordering2
    real(dp)     :: nullval, psi1, psi2
    real(dp)     :: fmissval = 0.
    logical(lgt) :: anynull
    ...
    open(48,file=trim(mapfile_out))
    do i = 0, npix-1
       if (mask(i,2) /= 0. .and. map1(i,2) /= -1.6375e30 .and. map2(i,2) /= -1.6375e30) then
          psi1 = atan2(map1(i,3),map1(i,2))/2 * 180./pi
          psi2 = atan2(map2(i,3),map2(i,2))/2 * 180./pi
          if (psi2-psi1 > 90.d0)  psi2 = psi2-180.d0
          if (psi2-psi1 < -90.d0) psi2 = psi2+180.d0
          write(48,*) psi1, psi2
       end if
    end do
  ...
  end subroutine print_pol_angles
...
990 close(unit1)
  ...
  subroutine expand_mask
    ...
    if (iargc() /= 4) then
       write(*,*) 'Usage: map_editor median_filter_source_holes [mask_in] [radius arcmin][mask_out]'
       stop
    end if
