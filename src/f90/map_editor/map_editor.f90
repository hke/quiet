program map_editor
  ...
  if (trim(operation) == 'scale' .or. trim(operation) == 'add_offset' .or. &
     ...
     call write_result_map(trim(outprefix)//'_stddev.fits', nside, ordering, header, map2, suffix=='_dp')
       ...
       & trim(operation) == 'splice' .or. trim(operation) == 'QU2P2_cross') then
     ...
     if (trim(beaminfo) == 'f2f') then
        ...
        call smooth_map(mapname_in1, r_fill, lmin, lmax, nside, ordering, nmaps, map, header, .false., .false., &
        ...
        call getarg(7,beamfile_in)
        call getarg(8,beamfile_out)
        call smooth_map(mapname_in1, r_fill, lmin, lmax, nside, ordering, nmaps, map, header, .false., .true., &
             & beamfile_in=beamfile_in, beamfile_out=beamfile_out)
  ...
  else if (trim(operation) == 'smooth_EB') then
     ...
     if (trim(beaminfo) == 'f2f') then
        ...
        call getarg(7,beamfile_in)
        call getarg(8,beamfile_out)
        call smooth_map(mapname_in1, r_fill, lmin, lmax, nside, ordering, nmaps, map, header, .true., .true., &
             & beamfile_in=beamfile_in, beamfile_out=beamfile_out)
     else if (trim(beaminfo) == 'f2g') then
        ! Input beam from file, output beam from gaussian
        call getarg(7,beamfile_in)
        ...
        call smooth_map(mapname_in1, r_fill, lmin, lmax, nside, ordering, nmaps, map, header, .true., .true., &
             & fwhm_in=fwhm_in, fwhm_out=fwhm_out)
     else if (trim(beaminfo) == 'f2f_QU') then
        ! Input beam from file, output beam from file
        call getarg(7,beamfile_in)
        call getarg(8,beamfile_out)
        call smooth_map(mapname_in1, r_fill, lmin, lmax, nside, ordering, nmaps, map, header, .true., .false., &
             & beamfile_in=beamfile_in, beamfile_out=beamfile_out)
     else if (trim(beaminfo) == 'f2g_QU') then
        ! Input beam from file, output beam from gaussian
        call getarg(7,beamfile_in)
        call getarg(8,string_real)
        read(string_real,*) fwhm_out
        call smooth_map(mapname_in1, r_fill, lmin, lmax, nside, ordering, nmaps, map, header, .true., .false., &
             & beamfile_in=beamfile_in, fwhm_out=fwhm_out)
     else if (trim(beaminfo) == 'g2f_QU') then
        ! Input beam from gaussian, output beam from file
        call getarg(7,string_real)
        read(string_real,*) fwhm_in
        call getarg(8,beamfile_out)
        call smooth_map(mapname_in1, r_fill, lmin, lmax, nside, ordering, nmaps, map, header, .true., .false., &
             & fwhm_in=fwhm_in, beamfile_out=beamfile_out)
     else if (trim(beaminfo) == 'g2g_QU') then
        ! Input beam from gaussian, output beam from gaussian
        call getarg(7,string_real)
        read(string_real,*) fwhm_in
        call getarg(8,string_real)
  ...
  else if (trim(operation) == 'output_harmonic_mode') then

     call output_harmonic_mode
  ...
  else if (trim(operation) == 'template_fit') then

     call template_fit

  else if (trim(operation) == 'pixconv') then

     call pixconv

  else if (trim(operation) == 'pixconv_firas') then
  ...
  else if (trim(operation) == 'print_pol_angles') then
     ...
     call print_pol_angles(mapname_in1, mapname_in2, maskfile, mapname_out)
  ...
  else if (trim(operation) == 'make_merged_ptsrc_map') then

     call make_merged_ptsrc_map
