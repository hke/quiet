module modulation_evidence
  use modulation_utils
  implicit none

contains

  subroutine compute_evidence(chain, unit, rng_handle, map, beam, & 
       & sigma_n, lmax_f, numsamp, fid_spectrum, l_pivot, l_mod, prior_f, prior_q, &
       & prior_n, pls, evidence)
    implicit none

    integer(i4b),                      intent(in)    :: chain, lmax_f, numsamp, unit, l_pivot, l_mod
    ...
    real(dp)         :: abs_eta, n_prop, q_prop, chisq, chisq_prop
    ...
    do i = 1, n
       ...
       ! Compute likelihood
       call compute_likelihood(data, covar_mat, ln_detC, L_x(i), chisq)
...
end module modulation_evidence
