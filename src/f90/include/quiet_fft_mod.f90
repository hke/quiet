...
module quiet_fft_mod
  ...
  interface fft_2d
     module procedure fft_outplace_cplx_dp_2d
  end interface
...
contains
  ...
  subroutine fft_outplace_cplx_dp_2d(tod_real, tod_fft, direction)
    implicit none
    real(dp),    dimension(0:,0:), intent(inout) :: tod_real
    complex(dpc), dimension(0:,0:), intent(inout) :: tod_fft
    integer :: direction, L, M
    integer*8 :: plan
    ...
    call fft_thread_init
    if (L/2+1 /= size(tod_fft(:,0)) .or. M /= size(tod_fft(0,:))) then
       write(*,*) 'quiet_fft_mod -- Incompatible array lengths in fft_outplace'
       stop
    end if

    if(direction > 0) then
       call dfftw_plan_dft_r2c_2d(plan, L, M, tod_real, tod_fft, &
            & fftw_estimate + fftw_unaligned)
       call dfftw_execute_dft_r2c(plan, tod_real, tod_fft)
       call dfftw_destroy_plan(plan)
       tod_fft = tod_fft/sqrt(real(L*M,dp))
    else
       call dfftw_plan_dft_c2r_2d(plan, L, M, tod_fft, tod_real, &
            & fftw_estimate + fftw_unaligned)
       call dfftw_execute_dft_c2r(plan, tod_fft, tod_real)
       call dfftw_destroy_plan(plan)
       tod_real = tod_real/sqrt(real(L*M,dp))
    end if
  end subroutine fft_outplace_cplx_dp_2d
