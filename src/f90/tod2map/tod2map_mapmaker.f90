...
!!$       write(58,*) i, data%N_d(i,1), assembly%tod(i,1)
!!$    end do
!!$    close(58)
!!$    call mpi_finalize(i)
!!$    stop
...
!!$    do i = 1, size(assembly%diode_info(1)%point(3,:,1))
!!$       write(58,*) i, assembly%diode_info(1)%point(3,i,1)
!!$    end do
!!$    close(58)
!!$    call mpi_finalize(i)
!!$    stop
    ...
    do di = 1, ndi
       ...
       do h = 1, ngroups
          ...
          !end do
          ! HKE -- 31/1/16 -- SPIDER convention; uK_cmb/ADU
          fact = quiet_diodes(adi)%stokes(stok) * quiet_horns(mod)%amps(h)
          do j = 1, nsamp
             data%phase(di)%val(:,h,j) = rphase(:,j) / assembly%diode_info(di)%gain(j) * fact
          end do
       end do
...
!             out%div(p,:,:) = out%div(p,:,:) + 1.d0/assembly%diode_info(d(1))%gain(i)**2
          end do
       end do
...
call bench_stop(info%bench, bench_cov4)
  ...
  subroutine build_cov(assembly, info, data, out)
    ...
    ! Don't do this -- only works for QUIET temperature assemblies
    if (.false. .and. all(info%comps == [T])) then
...
!!$end do
    ...
    !
    ! HKE 25/1/16: Disabled OMP below, as it gave catastrophic internal compiler error
    !
