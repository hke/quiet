...
contains
  ...
  subroutine read_l2_file(filename, data)
    ...
    !HKE, 29/1/16 -- disabled housekeeping for SPIDER analysis
...
! where should this sub logically be?
  ...
  subroutine read_l3_file(filename, data)
    ...
    type(lx_struct)              :: data
    type(hdf_file)               :: file
    integer(i4b) :: i, npix, nsamp, ndi, nmod, nobj, nbin, ext(7)
    call free_lx_struct(data)
...
!    do i = 1, size(data%point(1,:,1))
!       data%point(3,i,:) = data%point(3,i,:) + (i-1) !- pi/4.d0
!    end do
