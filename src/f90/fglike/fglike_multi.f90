program fglike
  ...
  logical(lgt) :: output_chain, log_delta_nu, fpa_norm
  ...
  real(dp)     :: prior_synch_bias, prior_dust_bias, sigma1, sigma2, norm, norm_ref, v1, v2, v3
  real(dp)     :: chain_delta_nu, chain_nu_min, T_0_sys, T_sys, T_sys_ref, nu_max_radio, noise_boost_radio
  real(dp), allocatable, dimension(:,:)  :: lnL, x, lnL_sub, summary, samples, L_prop, prior, F, cov, T_atm
  ...
  character(len=256) :: parfile, noise_model, outfile, atmofile
  ...
  call get_parameter(58, parfile, 'NU_CUT_HIGH', par_dp=nu_cut_high)
  call get_parameter(58, parfile, 'NUM_SETUP',  par_int=num_setup)
  call get_parameter(58, parfile, 'ATMOSPHERIC_PROFILE',  par_string=atmofile)
  call get_parameter(58, parfile, 'T_0_SYS',              par_dp=T_0_sys)
  call get_parameter(58, parfile, 'NU_MAX_RADIOMETERS',      par_dp=nu_max_radio)
  call get_parameter(58, parfile, 'NOISE_BOOST_RADIOMETERS', par_dp=noise_boost_radio)
  nu_max_radio = nu_max_radio*1d9
  call rand_init(handle, seed+100*myid)

  numcomp = 3
  allocate(A_ref(numcomp), beta_ref(3), nu_ref(numcomp), prior(3,2))
  ...
  call get_parameter(58, parfile, 'PRIOR_DUST_TD_RMS',    par_dp=prior(3,2))
  ...
  call get_parameter(58, parfile, 'OUTPUT_CHAIN',         par_lgt=output_chain)
  if (output_chain) then
     call get_parameter(58, parfile, 'CHAIN_DELTA_NU',       par_dp=chain_delta_nu)
     call get_parameter(58, parfile, 'CHAIN_NU_MIN',         par_dp=chain_nu_min)
  end if

  call get_parameter(58, parfile, 'FOCAL_PLANE_NORMALIZATION', par_lgt=fpa_norm)
  nu_ref     = nu_ref * 1d9
  prior(1,1) = beta_ref(1)+prior_synch_bias
  prior(2,1) = beta_ref(2)+prior_dust_bias
  prior(3,1) = beta_ref(3)
  burnin = 150000
  nu_rms = 100.d9   ! Arbitrary
  nu_cut_high = nu_cut_high * 1d9
  !if (myid == 0) output_chain = .true.

  call initialize_T_atm(atmofile, T_atm)
  ...
  do q = 1, num_delta_nu
     if (output_chain) then
        delta_nu = chain_delta_nu
     else if (log_delta_nu) then
     ...
     do i = 1, num_setup
        ...
        do j = 1, numband
           if (trim(noise_model) == 'uniform') then
              ...
              if (nu(j) > nu_cut_high) then
                 sig = A_ref(3) * compute_one_comp_dust_spectrum(nu(j), nu_ref(3), beta_ref(2), beta_ref(3)) 
                 sig_ref = A_ref(3) * compute_one_comp_dust_spectrum(nu_rms, nu_ref(3), beta_ref(2), beta_ref(3)) 
              else
                 sig = A_ref(1) / a2t(j) + &    ! CMB
                      & A_ref(2) * compute_synch_spectrum(nu(j), nu_ref(2), beta_ref(1)) + &
                      & A_ref(3) * compute_one_comp_dust_spectrum(nu(j), nu_ref(3), beta_ref(2), beta_ref(3)) 
                 sig_ref = A_ref(1) / compute_ant2thermo_single(nu_rms) + &    ! CMB
              ...
              if (allocated(T_atm)) then
                 T_sys     = T_0_sys + splint(T_atm(:,1), T_atm(:,1), T_atm(:,1), nu(j)*1d-9)
                 T_sys_ref = T_0_sys + splint(T_atm(:,1), T_atm(:,1), T_atm(:,1), nu_rms*1d-9)
                 rms(j)   = rms0 * (nu_rms/nu(j))**0.5d0 * T_sys/T_sys_ref
              else
                 rms(j)   = rms0 * (nu_rms/nu(j))**0.5d0
              end if
              if (nu(j) < nu_max_radio) then
                 rms(j) = rms(j) * noise_boost_radio
              end if
              if (sig > 1.d-30) then
           ...
           do j = 1+myid, numsim, numprocs
              ...
              do k = 1, numband
                 d(k) = A_ref(1) + &    ! CMB
                      ...
                      & A_ref(3) * a2t(k) * compute_one_comp_dust_spectrum(nu(k), nu_ref(3), beta_ref(2), beta_ref(3)) + &
                      & rms(k) * rand_gauss(handle)
              end do
              
              ! Run Markov chain until convergence
              p0     = [A_ref(1), A_ref(2), A_ref(3), beta_ref(1), beta_ref(2), beta_ref(3)]
              ...
              v1      = sqrt(variance(samples(4,burnin:k-1)))/prior(1,2)
              v2      = sqrt(variance(samples(5,burnin:k-1)))/prior(2,2)
              v3      = sqrt(variance(samples(6,burnin:k-1)))/prior(3,2)
              if (myid == 0) write(*,*) 'volume = ', real(v1,sp), real(v2,sp)
              deg_fac = max(v1,v2)
...
contains
  ...
  function compute_log_like(p, d, nu, rms, prior)
    ...
    do i = 1, size(d)
       if (nu(i) < nu_cut_high) then
          s = p(1) + &    ! CMB
               ...
               & p(3) * a2t(i) * compute_one_comp_dust_spectrum(nu(i), nu_ref(3), p(5), p(6))
    ...
    compute_log_like = compute_log_like - 0.5d0 * ((p(4)-prior(1,1))/prior(1,2))**2 - &
         & 0.5d0 * ((p(5)-prior(2,1))/prior(2,2))**2 - 0.5d0 * ((p(6)-prior(3,1))/prior(3,2))**2
  ...
  subroutine initialize_T_atm(atmofile, T_atm)  
    implicit none
    character(len=*), intent(in) :: atmofile
    real(dp), allocatable, dimension(:,:), intent(out) :: T_atm

    integer(i4b) :: n, i
    real(dp)     :: tmp

    if (trim(atmofile) /= 'none') then
       open(58,file=trim(atmofile))
       read(58,*) n
       allocate(T_atm(n,3))
       do i = 1, n
          read(58,*) T_atm(i,1), tmp, T_atm(i,2)
       end do
       close(58)
       call spline(T_atm(:,1), T_atm(:,2), 1.d30, 1.d30, T_atm(:,3))
    end if

  end subroutine initialize_T_atm
