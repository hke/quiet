program bscan
  ...
  real(dp)           :: theta_diode, psi_diode, phi_diode, el_diode, az_diode, dk_diode, vec_diode(3), mat(3,3), ra_tele, dec_tele
...
!           end if
           ...
           do while (mjd < flights(i)%mjd_start + flights(i)%duration)
              ...
              do k = 1, size(az_profile)-1
                 az_tele = az_profile(k)
                 call coord_convert(COORD_HOR, az_tele, el_tele, 0.d0, COORD_GAL, phi_tele, theta_tele, psi_tele, mjd=mjd+k/samprate/(3600.d0*24.d0))
                 ...
                 if (el_tele >= (90.d0-el_max)*DEG2RAD .and. el_tele <= (90.d0-el_min)*DEG2RAD .and. pdist <= psize) then
                    call coord_convert(COORD_HOR, az_tele, el_tele, 0.d0, COORD_CEL, ra_tele,  dec_tele, psi_tele, mjd=mjd+k/samprate/(3600.d0*24.d0))
...
!                    write(19,*) (mjd+k*dt-flights(i)%mjd_start)*3600*24, az_tele*RAD2DEG, (0.5d0*pi-el_tele)*RAD2DEG, acos(sum(vec2*vec_tele))*RAD2DEG
              ...
              mjd = mjd + size(az_profile) / samprate / (3600.d0*24.d0)
              time_since_repointing = time_since_repointing + size(az_profile) / samprate / (3600.d0*24.d0)
              deallocate(az_profile)

              ! Find new path
              az_limit = find_az_limit(mjd, psize, az_tele, el_tele, centerpath)
!              if (abs(az_limit-az_tele) < az_repoint_limit .or. time_since_repointing > 1.d0/24.d0) then
              if (time_since_repointing > cestime) then
                 write(*,*) mjd, 'repointing', time_since_repointing
                 call find_optimal_elevation(mjd, psize, centerpath, el_min, el_max, el_tele, cestime)
                 az_tele = splint(centerpath(:,1,1), centerpath(:,2,1), centerpath(:,3,1), mjd)
                 time_since_repointing = 0.d0
...
!                 end where
!                 call int2string(counter, ctext)
!                 call write_map(hitcount, 1, 'hitcount'//ctext//'.fits')
!                 hitcount = 0.d0
!                 close(19)
                 counter = counter+1
                 call int2string(counter,ctext)
                 open(19,file='scan'//ctext//'.dat', recl=1024)                 
                 write(*,*) 'counter = ', counter, el_tele
              end if
              call compute_az_profile(samprate, az_tele, az_limit, scanspeed, accel, turn_time, az_profile)
              !ok = abs(az_limit-az_tele) > az_repoint_limit
...
!     write(*,*) i, dec_min, dec_max
     ...
     write(*,*) 'Unmasked part in %', sum(mask)/real(12*nside**2-1,dp)*100.d0
  end do
