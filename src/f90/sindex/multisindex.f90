...
!!$  end subroutine leakage
  ...
  subroutine spectral_index(unit)
    ...
    real(dp),     allocatable, dimension(:,:)   :: outmap, areamap, uncert, chisq, multimap, multiuncert
    real(dp),     allocatable, dimension(:,:)   :: samlekart, samlekart2, samleuncert, samleuncert2, samlechisq, samlechisq2
...
write(*,*) 'A3 scatter areamap samlekart', size(areamap), size(samlekart)
    ...
    if (allocated(samlechisq)) deallocate(samlechisq)
    ...
    allocate(samlechisq(0:max_npix-1, nmaps))
    allocate(samlechisq2(0:max_npix-1, nmaps))
    multimap=healnan
    samlekart2=healnan
    multiuncert=healnan
    ...
    do ns = 0, lstop
       ...
       allocate(areamap(0:areanpix-1, nmaps))
       allocate(uncert(0:areanpix-1, nmaps))
       allocate(chisq(0:areanpix-1, nmaps))       
       ...
       do i = 0+myid, areanpix-1, numprocs
          ...
          call maxlikeplot(kart, var1, var2, locmask, pol, i, freq1, freq2, areamap(i,:), offset(i,:), uncert(i,:), chisq(i,:), fg(i*num:(i+1)*num-1, :))
    ...
    where(areamap == healnan) areamap = 0.d0
    where(uncert == healnan)  uncert  = 0.d0
    where(chisq == healnan)  chisq  = 0.d0
    ...
    if (.not. multi) then
       ...
       call mpi_reduce(chisq,  samlechisq2, size(chisq),mpi_double_precision, mpi_sum, root, MPI_COMM_WORLD, ierr)      
    end if
    ...
    allocate(samlechisq(0:npix-1, nmaps))
    samlekart = healnan
    samleuncert=healnan
    samlechisq=healnan
    if (ns_max == 0) then
    ...
    do k = 1, nmaps
       do i = 0, npix-1
          if (mask(i)==1) then
             ...
             samlechisq(i,k) = samlechisq2(i/ns, k)
          end if
       end do
    end do
    ...
    if (myid==0) then 
       where(samlekart   == 0.d0) samlekart   = healnan
       where(samleuncert == 0.d0) samleuncert = healnan
       where(samlechisq  == 0.d0) samlechisq = healnan
       ...
       outfile = trim(outprefix) // '_chisq.fits'
       call write_map(samlechisq, ordering, trim(outfile))
       write(*,*) '* chisquare values written to file = ', trim(outfile)
    ...
    if (allocated(samlechisq)) deallocate(samlechisq)
    if (allocated(samlechisq2)) deallocate(samlechisq2)
  ...
  subroutine maxlikeplot(map, sig1, sig2, mask, pol, index, freq1, freq2, spectral, offset, uncert, chisq, fg)
    ...
    real(dp),        dimension(:),     intent(out) :: spectral, offset, uncert, chisq
...
!    if (n<num/2) return
    ...
    ! else proceed
    if (pol==1) then
       call maxlike(map(:,1,1), map(:,1,2), sig1(:), sig2(:), freq1, freq2, spectral(1), offset(1), uncert(1), n, chisq(1), fg=fg(:,1),hpindex=index)
    else if (pol==2) then
       do k=2,3
          write(*,*) k
          call maxlike(map(:,k,1), map(:,k,2), sig1(1+num*(k-2):num*(k-1)), sig2(1+num*(k-2):num*(k-1)), freq1, freq2, spectral(k), offset(k), uncert(k), n, chisq(k), fg=fg(:,k), polbeta=spectral(1), poloff=offset(1))
       end do
       allocate(polmap(pol*num,2))
       polmap(1:num,:)       = map(:,2,:)
       polmap(num+1:2*num,:) = map(:,3,:)
       call maxlike(polmap(:,1), polmap(:,2), sig1(:), sig2(:), freq1, freq2, spectral(1), offset(1), uncert(1), n*pol, chisq(1), hpindex=index)
       deallocate(polmap)
    end if
  ...
  subroutine maxlike(map1, map2, sig1, sig2, freq1, freq2, spectral, offset, uncert, n, chisq, fg, polbeta, poloff, hpindex)

    real(dp),               dimension(:), intent(in)  :: map1, map2, sig1, sig2
    real(dp),                             intent(out) :: spectral, offset, uncert, chisq
    real(dp),                             intent(in)  :: freq1, freq2
