program tester
   ...
   !case("chisq_test"); call chisq_test
      case("make_ptsrc_map"); call make_ptsrc_map
...
contains
  ...
  subroutine make_ptsrc_map
    ...
    integer(i4b)       :: i, n, nside, npix, nmaps, unit, col, nlist, nsrc, nside2, npix2, myid, nproc, err
    ...
    real(dp),     allocatable, dimension(:,:) :: map, map2, buffer, buffer2
    real(dp),     allocatable, dimension(:)   :: data, src
    real(dp),     allocatable, dimension(:)   :: b_r, b_prof, b_prof2
    character(len=80), dimension(180)         :: header

    if (iargc() < 8) then
       write(*,*) 'Usage: map_editor make_ptsrc_map [ptsrc catalog] {WMAP,Planck,AT20G,nvss} [flux column] '
    ...
    allocate(map(0:npix-1,nmaps), listpix(0:npix-1), src(0:npix-1), buffer(0:npix-1,nmaps), map2(0:npix-1,nmaps))
    !allocate(map(0:npix-1,nmaps), listpix(0:10000), src(0:npix-1), map2(0:npix2-1,nmaps))
    if (trim(convention) == 'WMAP') then
       allocate(data(col+2))
...
!!$       stop
       ...
       allocate(data(7))
    else if (trim(convention) == 'AT20G') then
       allocate(data(3))
    else if (trim(convention) == 'nvss') then
       allocate(data(3))
    end if
    map = 0.d0
    map2 = 0.d0
    ...
    do while (.true.)
       if (trim(convention) == 'WMAP') then
          ...
          do i = 0, nlist-1
             call pix2vec_ring(nside, listpix(i), vec)
             r         = acos(sum(vec*cvec))
             src(i)    = exp(-0.5d0 * (r/sigma)**2)
          end do
       else if (trim(convention) == 'nvss') then
          read(unit,*,end=91) data
          nsrc = nsrc+1
          if (mod(nsrc+myid,nproc) /= 0) cycle
          if (mod(nsrc,1000) == 0) write(*,*) 'Processing source no. ', nsrc
          flux      = data(3)
          data(1:2) = data(1:2) * pi/180.d0
          call ang2vec(0.5d0*pi-data(2), data(1), cvec)
          call query_disc(nside, cvec, 3*fwhm, listpix, nlist)       
          src = 0.d0
          do i = 0, nlist-1
             call pix2vec_ring(nside, listpix(i), vec)
             r         = acos(sum(vec*cvec))
       ...
       if (trim(convention) /= 'nvss' .or. flux > 100.d0) then
          map(listpix(0:nlist-1),1) = map(listpix(0:nlist-1),1) + src(0:nlist-1)
       else
          map2(listpix(0:nlist-1),1) = map2(listpix(0:nlist-1),1) + src(0:nlist-1)
       end if
       !if (mod(nsrc,100) == 0) write(*,*) nsrc, sum(map), sum(map2)

       !if (nsrc >= 10000) exit
    end do
91  close(unit)

    call mpi_reduce(map, buffer, size(map), mpi_double_precision, mpi_sum, 0, mpi_comm_world, err)
    if (myid == 0) map = buffer
    !write(*,*) size(map2), size(buffer2), sum(map2), err
    call mpi_reduce(map2, buffer, size(map2), mpi_double_precision, mpi_sum, 0, mpi_comm_world, err)
    !write(*,*) 'final', size(map2), size(buffer2), sum(map2), err
    if (myid == 0) map2 = buffer
    ...
    !call write_minimal_header(header, 'MAP', nside=nside2, order=1, polar=.true.)
    !call write_result_map('lowflux_'//trim(outfile), nside2, 1, header, map)
       call write_map(map2, 1, 'lowflux_'//trim(outfile))
    end if
    !deallocate(map, map2, buffer)
    deallocate(map, buffer)
