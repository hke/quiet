...
call readDataAssignWork(pointing,timestream,myid,numprocs,produce_CES_maps,level2_dir, target_name, target_type, module_list,nside,alpha,sigma,fknee,ntod)
ndet_total = size(ntod)
...
do i=0,ndet_total-1
   rho(i,i)=1.0
enddo
call prepareFFTQ2(correlator,offsetLength,ntod,ndet_total,MPI_COMM_WORLD,sigma,fknee,alpha,rho,nyquist,.false.)
